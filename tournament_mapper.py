...
EXCLUDED_TOURNAMENTS = [
    'utr pro tennis', 'futures 2026', 'itf', 'juniors',
    'ultimate tennis showdown', 'rome 2',
]
...
class MatchCollector:
    ...
    def _get_ranking_date(self, date_str: str) -> str:
        """Retourne la date de classement la plus proche depuis la DB"""
        conn = get_connection()
        try:
            c = conn.cursor()
            # Cherche la date de classement la plus récente <= date du match
            match_date = date_str  # "2026-04-15"
            c.execute('''
    ...
    def _get_surface(self, tournament: str) -> str:
        ...
        for key, surface in SURFACE_OVERRIDES.items():
            if key in t_low:
                return surface
        self._unknown_tournaments.add(tournament)
        return 'Hard'
    ...
    def _print_unknown_tournaments(self):
        if self._unknown_tournaments:
            print(f"\n{'='*55}")
            print(f"⚠️  {len(self._unknown_tournaments)} tournois sans surface connue (fallback Hard) :")
            print(f"{'='*55}")
            for t in sorted(self._unknown_tournaments):
                print(f"   ❓ {t}")
            print(f"\n💡 Pour corriger, ajoute-les dans SURFACE_OVERRIDES.")
            print(f"{'='*55}\n")
        else:
...
if __name__ == "__main__":
    collector = MatchCollector(output_dir="data/csv")
    now = datetime.now()

    # ⚠️ Lance get_ranking.py d'abord si players_rankings est vide !

    # Option 1 — Une date
    # matches = collector.collect_date("2026-04-22", tours=['atp', 'wta'])

    # Option 2 — Mois en cours ATP uniquement
    # matches = collector.collect_month(now.year, now.month, tours=['atp'])

    # Option 3 — Mois en cours ATP + WTA
    matches = collector.collect_month(now.year, now.month, tours=['atp', 'wta'])

    # Option 4 — Plage de dates
    # matches = collector.collect_range("2026-03-01", "2026-04-22")

    # Option 5 — 30 derniers jours
    # matches = collector.collect_last_n_days(30)

    if matches:
        df = pd.DataFrame(matches)
        print(f"\n📊 Aperçu :")
        print(df[['date', 'tour', 'tournament', 'surface',
                   'player1', 'player2', 'winner', 'score',
                   'p1_rank', 'p2_rank']].head(15).to_string())
        print(f"\n🎾 ATP : {len(df[df['tour']=='ATP'])} matchs")
        print(f"🎾 WTA : {len(df[df['tour']=='WTA'])} matchs")
        print(f"🏟️  Surfaces : {df['surface'].value_counts().to_dict()}")
        ranked = df['p1_rank'].notna().sum()
        print(f"📈 Avec ranking : {ranked}/{len(df)} ({ranked/len(df)*100:.0f}%)")